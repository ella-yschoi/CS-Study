...
- 줄을 하나로 세우는 방법
- 한 개의 CPU가 대장 CPU가 되어 데이터의 접근과 공유를 책임짐
...
- 이러한 구조는 스레드 간의 통신을 용이하게 하고, 자원의 효율적 사용을 가능하게 함
...
- 글로벌 스케줄링은 커널 수준에서 이루어지며, 여기서는 운영체제가 스레드의 존재를 알 수 있으며, 커널의 스케줄러가 어떤 스레드를 실행할지 경정
